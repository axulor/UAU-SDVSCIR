...
def Game_simulation_IBRA(lower_net, upper_net):
    results = []  # 用于保存每次的结果
    lower_net = lower_net
    upper_net = upper_net
    # 创建博弈实例
    ...
    for t in range(global_t+1):
        ...
        if t == 0:
            ...
            Game_sim.update_strategy_IBRA(k=0.1) #策略选择
...
def Game_simulation_HSIRA(lower_net, upper_net):
    results = []  # 用于保存每次的结果
    lower_net = lower_net
    upper_net = upper_net
    # 创建博弈实例
...
if __name__ == "__main__":
    ...
    # 创建存储文件夹
    folder_name = "Again_fig3-IBRA"
    os.makedirs(folder_name, exist_ok=True)
    Sim = Game_simulation_IBRA(lower_net,upper_net)

    # 创建存储文件夹
    folder_name = "Again_fig3-SCRA"
    os.makedirs(folder_name, exist_ok=True)
    Sim_2 = Game_simulation_HSIRA(lower_net,upper_net)
