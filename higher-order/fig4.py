...
def Game_simulation_HSIRA(lower_net, upper_net,cost_v,tau):
    results = []  # 用于保存每次的结果
    lower_net = lower_net
    upper_net = upper_net
    # 创建博弈实例
    Game_sim = Game.Game(cost_v= cost_v,lower_net=lower_net,upper_net=upper_net,
                         alpha=0.5,
                         alpha_triangle=1.0,
                         delta=0.5,
                         delta_triangle=1.0,
...
if __name__ == "__main__":
    ...
    # 初始化cost_v的值
    cost_v_values = [i / 50.0 for i in range(1, 51)]
    ...
    for tau in [0.1, 0.5, 1.0]:
        ...
        with open(csv_file_path, mode='w', newline='') as file:
            ...
            # 遍历不同的cost_v值
            for cost_v in cost_v_values:
                # 运行游戏模拟并获取结果
                simulation_results = Game_simulation_HSIRA(lower_net, upper_net, cost_v, tau)
